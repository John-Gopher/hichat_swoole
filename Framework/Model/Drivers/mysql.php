...
class PipeMysql extends Object
{
    ...
    {
        $res = $this->query("select  column_name, column_comment ,column_key,extra ,column_type,is_nullable,column_default from Information_schema.columns  where table_Name = '{$table}'");
        if ($res !== false)
        {
            $arr = array();
            while ($row = mysql_fetch_assoc($res))
